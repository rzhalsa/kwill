                ...
                <v-col>
                    <div class="ability-grid ml-2">
                        ...
                        <div v-for="(slot, slotIndex) in 6" :key="slotIndex">
                            ...
                            <v-radio-group v-model="store.character_state.selections[slotIndex]" :rules="rule">
                                <v-radio
...
<script setup>
    ...
    // Rule for :rules in each v-radio-group for choosing ability scores
    const rule = [
        v => v !== null && v !== undefined && v !== '' || 'Req.'
    ]
    ...
    function saveAbilityScores() {
        ...
        if(store.character_state.selections.length === 6) {
            // Save each abiliy score value in the pinia store
            for(let i = 0; i < 6; i++) {
